...
# Create database if not exist. It is created in first found database server
$sqlServers = Get-AzureSqlDatabaseServer
$sqlServer = $sqlServers | Select-Object -First 1
...
$orderDatabase = Get-AzureSqlDatabase -ServerName $sqlServer.ServerName  -DatabaseName $ordersDatabaseName -ErrorVariable a -ErrorAction silentlycontinue

if ($orderDatabase -eq $null){
